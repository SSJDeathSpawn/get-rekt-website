...
class TeamForm(forms.ModelForm):
    ...
    def clean(self):
        if Team.objects.filter(name=self.cleaned_data['name'],game=self.cleaned_data['game']):
            
            raise forms.ValidationError("Team name already exists!")
        if Team.objects.filter(leader=self.cleaned_data['leader'],game=self.cleaned_data['game']):
    ...
    game = forms.ModelChoiceField(queryset=Game.objects.all(), widget=forms.RadioSelect(attrs={'class': 'text-white'}))
...
class AddMemberForm(forms.ModelForm):
    ...
    team=forms.ModelChoiceField(queryset=Team.objects.all(),widget=forms.RadioSelect(attrs={'class': 'text-white'}))
    ...
    def clean(self):
        ...
        if Student.objects.filter(regno=regno,team__game=self.cleaned_data['team'].game):
            raise forms.ValidationError("This person is already in a team for this game")
